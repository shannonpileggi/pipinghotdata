...
In a [previous post](https://www.pipinghotdata.com/titanic/exploring_variables/), we got an overview of Titanic's character variables.  Now let's see what we can do with the `Name` variable; recall, this variable has all unique values!  To get started, let's visually inspect a few values.
...
Fortunately, the `forcats` package has an awesome function that let's us do this quickly: `fct_lump`.  We're using `mutate` again to create a new title variable that is re-classified (`title2`).  The `fct_lump` function combines the least frequent values together in an `other` group, and the `n = 6` option specifies to keep the 6 most common values (so the 7th value is `other`).
...
Super, now `title2` is ready to use for analysis!
